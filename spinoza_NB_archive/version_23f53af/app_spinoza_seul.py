...
class DialogueSpinozaV2:
    ...
    def generate_response(self, user_input: str, history: List[Tuple] = None) -> Dict:
        ...
        # Logique climax pour résistance (code REF)
        climax_utilise = False
        if contexte == "resistance":
            if len(history) > 3:  # Au moins 3 échanges
                system_prompt += "\nRÉVÈLE LA CONTRADICTION MAJEURE. Sois percutant."
                climax_utilise = True
        ...
        for exchange in history[-4:]:
            ...
            if exchange[1]:  # assistant message
                # Nettoyer annotation contexte de la réponse précédente
...
def load_model():
    ...
    # Configuration 8-bit avec offload CPU/GPU
    quantization_config = BitsAndBytesConfig(
        load_in_8bit=True,  # CRITIQUE: 8-bit pas 4-bit
        llm_int8_threshold=6.0,
        llm_int8_has_fp16_weight=False,
        llm_int8_enable_fp32_cpu_offload=True,  # Permet offload CPU/GPU si pas assez de VRAM
    )
    ...
    base_model = AutoModelForCausalLM.from_pretrained(
        ...
        device_map="auto",
        dtype=torch.float16,  # Remplace torch_dtype (deprecated)
        token=HF_TOKEN,
...
def create_interface():
    ...
    def chat_function(message, history):
        """Fonction chat Gradio - SYSTÈME ADAPTATIF avec historique"""
        if not message.strip():
            return "", history

        try:
            # Utiliser l'historique fourni par le frontend (SYSTÈME ADAPTATIF)
            history = history or []
    ...
    with gr.Blocks(title="Spinoza Niveau B - V2") as interface:
        ...
        msg.submit(chat_function, [msg, chatbot], [msg, chatbot], api_name="/chat_function")
