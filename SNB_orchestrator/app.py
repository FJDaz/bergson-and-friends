...
# Lazy client: on ne le crée que quand on en a besoin
_client = None

def get_client():
    global _client
    if _client is None:
        # Retirer le paramètre timeout qui n'est pas supporté par cette version
        _client = Client(SNB_SPACE)
    return _client
...
def orchestrate(philosopher, message, history):
    ...
    # Client créé ici, pas au démarrage
    client = get_client()
    ...
    try:
        ...
        return f"Erreur: {str(e)}", history or []
