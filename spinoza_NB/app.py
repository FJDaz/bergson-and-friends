...
def load_model():
    ...
    quantization_config = BitsAndBytesConfig(
        ...
        # Pas d'offload CPU - T4 16GB devrait suffire pour Qwen 14B 8-bit (~14GB)
    )

    print("🔄 Chargement Qwen 14B (8-bit) sur GPU uniquement...")
    
    # T4 a 16GB VRAM - Qwen 14B 8-bit fait ~14GB
    # Limiter à GPU uniquement (pas de CPU/disk) pour bitsandbytes
    base_model = AutoModelForCausalLM.from_pretrained(
        BASE_MODEL,
        quantization_config=quantization_config,
        device_map="auto",  # Auto mais limité à GPU via max_memory
        dtype=torch.float16,
        token=HF_TOKEN,
        trust_remote_code=True,
        max_memory={0: "15GiB", "cpu": "0GiB"}  # Forcer tout sur GPU, rien sur CPU
    )
